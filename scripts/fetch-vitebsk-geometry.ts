/**
 * Получение полигонов водоёмов Витебской области (1–279) из OpenStreetMap.
 * Заменяет прямоугольные полигоны на реальные контуры из OSM.
 */
...
const INPUT_FILE = path.join(ROOT, 'vitebsk_waterbodies.geojson');
const OUTPUT_FILE = path.join(ROOT, 'vitebsk_waterbodies.geojson');

// Витебская область: юг, запад, север, восток
const VITEBSK_BBOX = '54.4,26.4,56.3,31.6';
...
function getCentroid(f: GeoFeature): { lat: number; lon: number } | null {
...
async function fetchAllWater(): Promise<OverpassElement[]> {
  const query = `
    ...
    (
      way["natural"="water"](${VITEBSK_BBOX});
      relation["natural"="water"]["type"="multipolygon"](${VITEBSK_BBOX});
    );
    out geom;
  `;
...
async function fetchAllRivers(): Promise<OverpassElement[]> {
  const query = `
    [out:json][timeout:120];
    (
      way["waterway"="river"](${VITEBSK_BBOX});
      way["waterway"="stream"]["intermittent"!="yes"](${VITEBSK_BBOX});
      way["waterway"="canal"](${VITEBSK_BBOX});
    );
    out geom;
  `;
  const data = await queryOverpass(query);
...
function findBestWater(
  ...
  const nameParts = nameHint.toLowerCase().replace(/[^\p{L}\s]/gu, '').split(/\s+/).filter((s) => s.length >= 2);
  const score = (e: OverpassElement) => {
    ...
    for (const part of nameParts) {
      ...
      if (part.length >= 4 && (name.includes(part) || part.includes(name))) s += 100;
    }
  ...
  if (!c || dist(targetLat, targetLon, c.lat, c.lon) > 0.25) return null;
  return best;
}
...
async function main() {
  ...
  console.log(`Обновление геометрии для ${toUpdate.length} водоёмов (id 1–279)...\n`);

  let waterElements: OverpassElement[] = [];
  let riverElements: OverpassElement[] = [];

  try {
    console.log('  Запрос водоёмов из OSM...');
  ...
  for (const f of toUpdate) {
    ...
    if (newGeom && newGeom.type !== 'LineString') {
      f.geometry = newGeom;
  ...
  console.log(`\nОбновлено ${updated} из ${toUpdate.length}. Файл: ${OUTPUT_FILE}`);
}
